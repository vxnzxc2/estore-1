...
interface Props {
  ...
  grandTotal: number
  deliveryFee: number
  onConfirm: (method: string, fulfillment: string, payLaterTerm?: number) => void
  onCancel: () => void
  light?: boolean
}
...
const PAYMENT_METHODS = [
  ...
  { id: 'later', label: 'Pay Later',     icon: Clock,      color: 'text-orange-500', bg: 'bg-orange-50 dark:bg-orange-500/10',desc: 'Split into up to 3 monthly payments'},
]
...
export default function PaymentModal({ total, grandTotal, deliveryFee, onConfirm, onCancel, light }: Props) {
  ...
  const [payLaterTerm,  setPayLaterTerm]  = useState(1)
  ...
  const payLaterAmount = fulfillment === 'pickup' ? total : grandTotal
  const payLaterMonthly = payLaterAmount / payLaterTerm + payLaterAmount * 0.0395

  // When Continue is pressed: if method is 'later' open PayLaterModal, otherwise go to confirm step
  ...
  // Called when user confirms their term inside PayLaterModal
  const handlePayLaterConfirm = (term: number) => {
    setShowPayLater(false)
    onConfirm('later', fulfillment, term)
  }
  ...
  return (
    <>
          ...
          <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
            ...
            {step === 'method' ? (
              <>
                  ...
                  <div>
                    ...
                    {pinCoords && (
                      <p className={`text-[10px] ${sub} mt-1.5 font-mono flex items-center gap-1`}>
                        ...
                        <button type="button" onClick={() => setPinCoords(null)} className="ml-1 text-red-400 hover:text-red-500">✕</button>
                      </p>
                    )}
                  </div>
                )}
                ...
                {method === 'later' && (
                  <div className={`rounded-2xl border ${light ? 'border-gray-200 bg-gray-50' : 'border-white/10 bg-slate-900/60'} p-4 space-y-3`}>
                    <div className="flex items-center justify-between">
                      <p className={`text-xs font-semibold ${sub} uppercase tracking-wider`}>Pay Later term</p>
                      <span className="text-amber-500 text-xs font-semibold">Interest 3.95% / month</span>
                    </div>
                    <div className="flex gap-2">
                      {[1, 2, 3].map(opt => (
                        <button key={opt} type="button" onClick={() => setPayLaterTerm(opt)}
                          className={`flex-1 rounded-2xl py-3 text-sm font-semibold transition-colors ${payLaterTerm === opt ? 'bg-amber-500 text-white' : 'bg-transparent border border-gray-200 text-gray-700 hover:bg-gray-100'}`}>
                          {opt} month{opt > 1 ? 's' : ''}
                        </button>
                      ))}
                    </div>
                    <div className={`text-xs ${sub}`}>
                      Estimated monthly payment: <span className="font-semibold text-amber-500">₱{payLaterMonthly.toFixed(2)}</span>
                    </div>
                  </div>
                )}
              ...
              /* Confirm step — only reached for non-later methods */
      ...
      {/* Pay Later modal — opens automatically when user picks 'later' and taps Continue */}
      {showPayLater && (
        <PayLaterModal
          total={fulfillment === 'pickup' ? total : grandTotal}
          initialTerm={payLaterTerm}
