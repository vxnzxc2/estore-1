...
function CartItemRow({ item, selected, onSelect, onQtyChange, light }: {
  ...
  const handleInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    onQtyChange(item.id, Math.max(1, raw === '' ? 1 : parseInt(raw)))
  }
  ...
  return (
    <div className={`flex items-center gap-2 ${row} rounded-xl p-2.5 border transition-colors`}>
      <button onClick={onSelect} className="shrink-0 text-amber-500">
        {selected ? <CheckSquare size={18} strokeWidth={2} /> : <Square size={18} strokeWidth={1.5} className={light ? 'text-gray-300' : 'text-slate-600'} />}
      </button>
      <div className="w-11 h-11 rounded-lg overflow-hidden bg-gray-100 shrink-0">
        {imgError ? <div className="w-full h-full flex items-center justify-center text-gray-300"><ShoppingBag size={14} strokeWidth={1.5} /></div>
      ...
      <div className={`flex items-center ${qtyBox} border rounded-lg overflow-hidden shrink-0`}>
        <button onClick={() => onQtyChange(item.id, Math.max(1, item.qty - 1))}
          className={`w-7 h-7 flex items-center justify-center ${qtyBtn} transition-colors text-sm`}>−</button>
...
function ConfirmDialog({ count, onConfirm, onCancel, light }: { count: number; onConfirm: () => void; onCancel: () => void; light?: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className={`${bg} border rounded-2xl p-6 w-full max-w-xs animate-fade-up`}>
        <div className="flex flex-col items-center gap-3 text-center">
          <div className="w-12 h-12 rounded-full bg-red-500/10 flex items-center justify-center">
            <AlertTriangle size={24} className="text-red-500" strokeWidth={2} />
          </div>
          <div>
            <p className={`font-bold ${title} text-base`} style={{ fontFamily: 'Syne, sans-serif' }}>Remove {count} item{count !== 1 ? 's' : ''}?</p>
            <p className={`${sub} text-sm mt-1`}>Are you sure you want to remove the selected items?</p>
          </div>
          <div className="flex gap-2 w-full mt-1">
            <button onClick={onCancel} className={`flex-1 py-2.5 rounded-xl text-sm font-medium ${light ? 'bg-gray-100 hover:bg-gray-200 text-gray-700' : 'bg-slate-700 hover:bg-slate-600 text-slate-300'} transition-colors`}>Cancel</button>
            <button onClick={onConfirm} className="flex-1 py-2.5 rounded-xl text-sm font-semibold bg-red-500 hover:bg-red-400 text-white transition-colors">Yes, Remove</button>
          </div>
        </div>
      </div>
    </div>
...
export default function CartSidebar({ cart, onQtyChange, onRemove, onRemoveSelected, onClear, onClose, onPlaceOrder, light }: Props) {
  ...
  const selBar = light ? 'bg-red-50 border-red-100' : 'bg-red-500/10 border-red-500/20'
        ...
        <div className={`animate-slide-right relative ${panel} w-full max-w-sm h-full flex flex-col shadow-2xl border-l ${hdr} overflow-hidden`}>
          ...
          <div className={`flex items-center justify-between px-4 py-3.5 border-b ${hdr} shrink-0`}>
            ...
            <div className="flex items-center gap-1.5">
              {cart.length > 0 && (
                <button onClick={toggleAll} className={`text-xs font-medium px-2.5 py-1.5 rounded-lg transition-colors ${light ? 'bg-gray-100 hover:bg-gray-200 text-gray-600' : 'bg-slate-700 hover:bg-slate-600 text-slate-300'}`}>
          ...
          {/* Delete selected bar */}
          {selected.size > 0 && (
            <div className={`flex items-center justify-between px-4 py-2 border-b ${hdr} ${selBar} shrink-0`}>
            ...
            <div className={`${footer} px-4 py-4 flex flex-col gap-3 shrink-0`}>
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  <span className={sub}>Subtotal</span><span className={sub}>₱{total}</span>
                </div>
                <div className="flex items-center justify-between text-xs">
